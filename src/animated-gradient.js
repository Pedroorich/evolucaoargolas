// ==========================================================================
// ANIMATED GRADIENT - WEBGL2 COMPONENT (SPELL-UI)
// Shader GLSL nativo com presets e efeito swirl de alta performance
// ==========================================================================
...
export function initAnimatedGradient(containerSelector, userConfig = {}) {
  ...
  const basePreset = presets[userConfig.preset] || presets.Prism;
  const params = {
    ...basePreset,
    ...userConfig,
    color1: userConfig.color1 ?? basePreset.color1,
    color2: userConfig.color2 ?? basePreset.color2,
    color3: userConfig.color3 ?? basePreset.color3,
    rotation: userConfig.rotation ?? basePreset.rotation,
    speed: userConfig.speed ?? basePreset.speed,
    swirl: userConfig.swirl ?? basePreset.swirl,
    swirlIterations: userConfig.swirlIterations ?? basePreset.swirlIterations,
    distortion: userConfig.distortion ?? basePreset.distortion,
    proportion: userConfig.proportion ?? basePreset.proportion,
    scale: userConfig.scale ?? basePreset.scale,
    softness: userConfig.softness ?? basePreset.softness,
    offset: userConfig.offset ?? basePreset.offset,
    shape: userConfig.shape ?? basePreset.shape,
    shapeSize: userConfig.shapeSize ?? basePreset.shapeSize,
  };
  ...
  if (!gl) {
    console.warn("WebGL2 não suportado para Animated Gradient");
    return null;
  }
  ...
  const animate = (time) => {
    ...
    gl.uniform1f(uniforms.u_shape, PatternShapes[params.shape] ?? 0);
